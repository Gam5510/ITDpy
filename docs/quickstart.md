...
Самый простой способ начать — авторизоваться по email и паролю. SDK сам
пройдёт капчу Cloudflare Turnstile через браузер, авторизуется и получит
`refresh_token` — вручную ничего копировать из DevTools не нужно. Подробности
механизма — в разделе [Turnstile Interceptor](../README.md#-turnstile-interceptor).
...
## Альтернатива: вход по сохранённому refresh token

Если у вас уже есть сохранённый `refresh_token` (например, с прошлой сессии
или для CI-окружения без браузера), можно передать его напрямую — тогда
браузер вообще не запускается.
